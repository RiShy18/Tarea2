...
gameControls:
	...
	jz .nokeyad
		...
		cmp byte [pressR], 1 ;try to move x+1 if 'd' is pressed and set animation accordingly, test other cases otherwise
		jne .nd
		inc cx
		mov bp, playerImg_right
		.nd:
		cmp byte [pressL], 1 ;try to move x-1 if 'a' is pressed and set animation accordingly, test other cases otherwise
		jne .na
		dec cx
	...
	jz .nokeyws
		...
		mov dx, word [player_PosZ] ;set dx to player z
		mov bp, [player]           ;set bp to current animation
		cmp byte [pressU], 1 ;try to move z-1 if 'w' is pressed and set animation accordingly, test other cases otherwise
		jne .nw
		dec dx
		mov bp, playerImg_back
		.nw:
		cmp byte [pressD], 1 ;try to move z+1 if 's' is pressed and set animation accordingly, test other cases otherwise
		jne .ns
		inc dx
...
keyboardINTListener: ;interrupt handler for keyboard events
	pusha	
		...
		cmp al,0x4b ;a- left
		jne .check1         
			mov byte [cs:pressL], bl ;use cs overwrite because we don't know where the data segment might point to
		.check1:
		cmp al,0x4D ;d- right
		jne .check2
...
playerImg_back_0  incbin "img/player_up.bin"
playerImg_back_1  incbin "img/player_up.bin"
playerImg_back_2  incbin "img/player_up.bin"
playerImg_right_0 incbin "img/player_right.bin"
playerImg_right_1 incbin "img/player_right.bin"
playerImg_right_2 incbin "img/player_right.bin"
playerImg_left_0  incbin "img/player_left.bin"
